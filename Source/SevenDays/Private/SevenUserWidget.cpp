...
void USevenUserWidget::ShowGameOverUI()
{
	...
	if (PC)
	{
		TSubclassOf<UGameOverWidget> SevenHUDClass = LoadClass<UGameOverWidget>(nullptr, TEXT("/Game/UI/BP_GameOverWidget.BP_GameOverWidget_C"));
		if (SevenHUDClass)
		{
			GameOverWidget = (CreateWidget<UGameOverWidget>(PC, SevenHUDClass));
			if (GameOverWidget)
			{
